...
//SHIT FOR THE EDGE DETECTION
//NOTE: NOT SURE WHICH PIN IS WHAT
#define EDGE_TRIG_PIN  A1   //originally left
#define EDGE_ECHO_PIN  13     //originally right
...
float durationl, durationr, cml, cmr;
...
void setup() {
  ...
  //Edge detection set up
  pinMode(EDGE_TRIG_PIN, OUTPUT);
  pinMode(EDGE_ECHO_PIN, INPUT);
...
//  }
  ...
  digitalWrite(EDGE_TRIG_PIN,LOW);
  delay(0.1);
  digitalWrite(EDGE_TRIG_PIN,HIGH);
  delay(0.1);
  digitalWrite(EDGE_TRIG_PIN,LOW);
  

  durationl = pulseIn(EDGE_ECHO_PIN, HIGH);
...
void backupAndTurnLeft()
{
  ...
  //Turn left
  //shit takes huge turn to avoid edge (essentially 175 turn)
    leftMotor1.run(3); rightMotor1.run(3);//3-> left
    leftMotor2.run(3); rightMotor2.run(3);
    leftMotor1.setSpeed(200); rightMotor1.setSpeed(200);
    leftMotor2.setSpeed(200); rightMotor2.setSpeed(200);
...
//      cml = (durationl/2) * 0.0343;
//      cmr = readPing();
//      Serial.print("left sensor while turning: ");
//      Serial.println(cml);
//      Serial.print("right sensor while turning: ");
//      Serial.println(cmr);
//    }
  }
    timer = millis()+2000;
...
void turn() {
  ...
  if (Sleft <= TURN_DIST && Sright <= TURN_DIST) {
    ...
    //      leftMotor1.setSpeed(50); rightMotor1.setSpeed(50);
    //      leftMotor2.setSpeed(50); rightMotor2.setSpeed(50);
    //    }
    //    else {
    leftMotor1.run(3); rightMotor1.run(3);//3-> left
    leftMotor2.run(3); rightMotor2.run(3);
